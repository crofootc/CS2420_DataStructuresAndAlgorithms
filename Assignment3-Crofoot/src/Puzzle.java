...
//        System.out.println(initNode.toString());
        ...
        // This is the Solution using an ArrayList
//        Node currentNode[] = new Node[1];
//        currentNode[0] = initNode;
//        ArrayList<Node[]> nodeList = new ArrayList<Node[]>();
//        nodeList.add(currentNode);
//        ArrayList<Integer> hashList = new ArrayList<>();
//        Integer tempHash;
//        while (solution == null){
//            ArrayList<Node[]> temp = nodeList;
//            nodeList = new ArrayList<>();
//            for (int i = 0; i < temp.size(); i++){
//                if (solution != null){break;}
//                for (int j = 0; j < (temp.get(i)).length; j++){
//                    if (temp.get(i)[j].isGoal()){
//                        solution = temp.get(i)[j];
//                        break;
//                    }
//                    tempHash = temp.get(i)[j].hashCode();
//                    if (!hashList.contains(tempHash)) {
//                        hashList.add(tempHash);
//                        Node[] expanded = temp.get(i)[j].expand();
...
//        }
        ...
        puzzleList.insert(initNode);

        while (solution == null){
            if (puzzleList.isSolved()){
                solution = puzzleList.getSolution();
                break;
            }
            puzzleList.expandList();
        }

//        System.out.print("\n\nSOLUTION  of Depth " + solution.getDepth() + "\n");
//        System.out.println(" Total Nodes Expanded " + allNodes.size() + "\n");
        System.out.print("\n\n====SOLUTION====\n");
        System.out.println("Number of states put on the queue: " + puzzleList.getNumInList());
...
//        System.out.println(initNode.toString());
        ...
        // This is the solution using an AVL tree as a priority queue
        // A priority queue has insertions made in any order but remove in order of prioirty
        // Priority is determined on estimation of work needed to solve the problem
        // Expected work function is determine by checking the amount of cars between the goal_car and goal

        AVLTree<Node> puzzleList = new AVLTree();
        AVLTree<Integer> hashList = new AVLTree();
        int numOfStates = 0;
        ...
        while (solution == null) {
            Node[] expanded = puzzleList.findMin().expand();

            for (int i = 0; i < expanded.length; i++) {
//                if (!hashList.contains(expanded[i].hashCode())) {
                if (!hashList.contains(Objects.hash(expanded[i]))) {

                    if (expanded[i].isGoal()) solution = expanded[i];
//                    hashList.insert(expanded[i].hashCode());
                    hashList.insert(Objects.hash(expanded[i]));
                    puzzleList.insert(expanded[i]);
                    numOfStates++;
                }
            }
            puzzleList.deleteMin();
        }
...
//        System.out.println(" Total Nodes Expanded " + allNodes.size() + "\n");
        System.out.print("\n\n====SOLUTION====\n");
        System.out.println("Number of states put on the queue: " + numOfStates);
        printSolution(solution);
    }
