...
public class Node implements Comparable<Node>{
    ...
    private int moves_left;     //Estimated moves left to solve puzzle
    private int priority;       //Depth + estimated number of remaining moves to reach a solution
    ...
    private boolean expanded;   //Returns False if the node has not yet been expanded
     ...
     */
     Node(Node parent, String history, Puzzle puzzle, int[] varPos, String move, int depth
        ...
        this.move = move;
        this.depth = depth;
        this.moves_left = getCarsInBetweenGoal();
        this.priority = depth + moves_left;
        getGrid();
        computeHashCode();
    }
    ...
    // This function determines the car inbetween the goal car and the goal
    public int getCarsInBetweenGoal(){
         int start = varPos[0] + puzzle.getCarSize(0);
         int cars = 0;
         while (start < puzzle.getGridSize()){
             for (int i = 1; i < puzzle.getNumCars(); i ++){
                 if (puzzle.getCarOrient(i) && puzzle.getFixedPosition(i) == start){
                     if (puzzle.getFixedPosition(0) >= varPos[i] && puzzle.getFixedPosition(0) < varPos[i]+puzzle.getCarSize(i)){
                         cars++;
                         break;
                     }
                 }
    ...
    public int getMoves_left(){return this.moves_left;}
    ...
    public int compareTo(Node n2){
        if (this.priority < n2.getPriority()) return -1;
        if (this.priority > n2.getPriority()) return 1;
        return 0;
    }
